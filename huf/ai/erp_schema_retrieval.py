...
def get_ai_settings():
    defaults = {
        ...
        "enable_safe_erp_tools": True,
        "safe_tool_record_limit": 500,
        "safe_tool_display_limit": 20,
        "safe_tool_max_scan_records": 5000,
        "preferred_agent_model": None,
        "preferred_smart_model": None,
        "preferred_fast_model": None,
        "use_smart_model_for_analytics": True,
        "answer_verbosity": "مختصر",
    ...
    try:
        if frappe.db.exists("DocType", "HUF AI Settings"):
            ...
            defaults.update({
                ...
                "enable_safe_erp_tools": bool(getattr(doc, "enable_safe_erp_tools", 1)),
                "safe_tool_record_limit": int(getattr(doc, "safe_tool_record_limit", 500) or 500),
                "safe_tool_display_limit": int(getattr(doc, "safe_tool_display_limit", 20) or 20),
                "safe_tool_max_scan_records": int(getattr(doc, "safe_tool_max_scan_records", 5000) or 5000),
                "preferred_agent_model": getattr(doc, "preferred_agent_model", None),
                "preferred_smart_model": getattr(doc, "preferred_smart_model", None),
                "preferred_fast_model": getattr(doc, "preferred_fast_model", None),
                "use_smart_model_for_analytics": bool(getattr(doc, "use_smart_model_for_analytics", 1)),
                "answer_verbosity": getattr(doc, "answer_verbosity", None) or "مختصر",
                "default_reasoning_effort": getattr(doc, "default_reasoning_effort", None) or "medium",
                "default_text_verbosity": getattr(doc, "default_text_verbosity", None) or "low",
